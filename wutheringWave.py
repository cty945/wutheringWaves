    ...
    for discard_idx in range(crit1DiscardChecks):
        ...
        for t in range(targetAmount):
            ...
            while not doubleCRIT:
                ...
                if 0 in echo[discard_idx+1: checkLevel + 1] or 1 in echo[discard_idx+1: checkLevel + 1]:
                    doubleCRIT = True
...
data = [[[float(item[0]), float('{:.2f}'.format(float(item[1])/10)), float(item[2])] for item in sublist] for sublist in crit1Expectation]
